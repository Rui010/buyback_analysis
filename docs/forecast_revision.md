...
| `net_income` | 当期純利益 / 親会社株主に帰属する当期純利益 / 親会社の所有者に帰属する当期利益 | ProfitAndLoss.net_income |
| `net_income_total`（v3.0追加） | 当期利益（IFRSで非支配持分を含む全体。親会社帰属分と別行で開示される場合のみ） | —（対応カラムなし。IFRS企業の非支配持分込み利益を欠落させないための別建て指標） |
