...
1. **`insert_skipped.sql` を実行して `is_checked` に登録する**（未実行）
   ```bash
   sqlite3 data.db < logs/insert_skipped.sql
   ```
2. `main.py` の事前チェック（コミット `a205103`）により、登録後は再試行しない。
...
| # | 内容 | 優先度 | 関連Issue |
|---|------|--------|-----------|
| 1 | `logs/insert_skipped.sql` を実行して214件を `is_checked` に登録 | 高 | Issue B |
| 2 | Issue A の対応方針を決定（A/B/C のいずれか） | 高 | Issue A |
| 3 | `retirement.md` プロンプト改善 + `REQUIRED_FIELDS` 追加（retirement_date） | 中 | Issue D |
| 4 | `correction.md` プロンプト改善 + `REQUIRED_FIELDS` 追加（original_announcement_date） | 中 | Issue D |
| 5 | モデルの PK 修正（`resolution_date` を PK から除外） | 中 | Issue E |
| 6 | `completion.py` の型不整合修正（buyback_method, shares_acquired） | 中 | Issue E |
| 7 | `skipped_comparison.csv` の77件差異を確認し、DBの値が誤っていれば UPDATE SQL を作成 | 低 | Issue A |
| 8 | 5038・3626 等の「補足説明資料」「取得事項の一部変更」の扱いを決定 | 低 | Issue A |
