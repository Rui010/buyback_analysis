...
2. `.env` ファイルを編集し、以下の値を設定します
   ...
   | `RERUN_URLS` | 強制再処理するURLをカンマ区切りで指定。既存データを削除して再パースする | - |
...
プロンプトを変更した際など、特定のURLのデータを削除して再パースしたい場合は `RERUN_URLS` を使います。

```bash
# 1件
RERUN_URLS=https://example.com/doc.pdf python -m buyback_analysis.main

# 複数件（カンマ区切り）
RERUN_URLS=https://example.com/doc1.pdf,https://example.com/doc2.pdf python -m buyback_analysis.main
```

`RERUN_URLS` を指定すると、日付範囲クエリは行わず指定URLのみを対象に処理します。対象URLの既存データ（IRテーブル・`is_checked`）は処理前に削除されるため、detect_type の再判定からパース・保存まで一通り再実行されます。

> **Windows PowerShell の場合：**
> ```powershell
> $env:RERUN_URLS = "https://example.com/doc.pdf"; python -m buyback_analysis.main
> ```
...
自社株買いパイプラインと同じ環境変数（`SQLITE_DB_URL`, `POSTGRESQL_DB_*`, `GEMINI_API_KEY`, `PDF_DOWNLOAD_PATH`, `DAYS_BACK`, `SYSTEM_START_DATE`, `SYSTEM_END_DATE`, `RERUN_URLS`）を使用します。
