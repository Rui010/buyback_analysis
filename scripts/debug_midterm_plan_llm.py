"""
中期経営計画 LLM 応答 Dry run スクリプト

PostgreSQL から直接データを取得し、DB への書き込みなしで
parse_text_by_llm() の返り値を確認する。

使い方:
    python -m scripts.debug_midterm_plan_llm          # 直近3件
    python -m scripts.debug_midterm_plan_llm --limit 5
    python -m scripts.debug_midterm_plan_llm --url https://...
"""
...
def run(limit: int, url: str | None) -> None:
    ...
    if url:
        ...
        df = get_tdnet_midterm_data(pg_engine, start_date=start_date, end_date=end_date)
