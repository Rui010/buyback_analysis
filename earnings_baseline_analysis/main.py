...
os.environ.setdefault("LOG_FILE", "earnings_baseline_analysis.log")
...
from earnings_baseline_analysis.models.earnings_baseline_metric import EarningsBaselineMetric  # noqa: F401 init_db() に登録するためインポート
...
from earnings_baseline_analysis.usecase.post_earnings_baseline import post_earnings_baseline
from earnings_baseline_analysis.usecase.notify_earnings_baseline_corrections import notify_earnings_baseline_corrections
...
def _already_exists(session, code: str, url: str) -> bool:
    return session.get(EarningsBaseline, {"code": code, "url": url}) is not None
...
def _determine_extraction_status(obj: dict | None) -> str:
    if obj is None:
        return "failed"
    inner = obj.get("data", {})
    actual_metrics = inner.get("actual_metrics") or []
    initial_forecast_metrics = inner.get("initial_forecast_metrics") or []
    has_data = (
        any(m.get("value") is not None for m in actual_metrics)
        or any(m.get("value") is not None for m in initial_forecast_metrics)
    )
    ...
    try:
        ...
        if RERUN_URLS:
            ...
            for url in RERUN_URLS:
                session.query(EarningsBaselineMetric).filter(EarningsBaselineMetric.url == url).delete()
                session.query(EarningsBaseline).filter(EarningsBaseline.url == url).delete()
            session.commit()
            df = get_tdnet_earnings_baseline_data_by_urls(engine=postgresql_engine, urls=RERUN_URLS)
        else:
            df = get_tdnet_earnings_baseline_data(
        ...
        notify_error("earnings_baseline_analysis", str(e))
        raise
