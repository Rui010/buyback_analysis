...
添付されたPDFは自己株式取得に関する途中経過について書かれています。
...
- `cumulative_shares_acquired` と `cumulative_amount_spent_yen` は、文書に `0` と明示されている場合のみ `0` を返してください。数値が見つからない・読み取れない場合は `0` ではなく `null` を入れてください。
