...
def extract_midterm_keywords(title: str, content: str, code: str, name: str) -> Optional[Dict[str, Any]]:
    """
    中期経営計画PDFのテキストから、戦略テーマ・重点施策のキーワードをresponseSchemaで
    型固定して抽出する。metrics抽出（既存の`midterm_plan.md`）とは別コールにする。
    ...
    prompt = load_prompt_template("midterm_keywords.md", title=title, content=content, code=code, name=name)
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if e.code in {502, 503, 504}:
                logger.info(
                    f"[Keywords] サーバーエラー {e.code}が発生しました。リトライ {attempt}/{max_retries}"
                )
